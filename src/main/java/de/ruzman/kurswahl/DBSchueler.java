...
public class DBSchueler extends DBVerbindung
{
    ...
    public boolean belegtFach(String fach)
    {
        ...
        {
            // SQL-Statement:
            rs = stmt.executeQuery("SELECT Count(" + Konfig.KURS_FACHBEZEICHNUNG +
                    ") FROM " + Konfig.KURS_TABELLE + " WHERE " +
                    Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() + "' AND " +
                    Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%' AND " +
                    Konfig.KURS_FACHBEZEICHNUNG + " LIKE '" + fach + "'");
            rs.next();
    ...
    public String gibBezeichung()
    {
        ...
        {
            // SQL-Statement:
            rs = stmt.executeQuery("SELECT " + Konfig.KURS_SCHULFORMEN +
                    " FROM " + Konfig.KURS_TABELLE + " WHERE " +
                    Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() +
                    "' AND " + Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%'");
            rs.next();
            // Bezeichnung:
     ...
     * der Schüler besucht.
     *
     * @param fach Fach, wo der Lehrer zu ermitteln ist
    ...
    public String gibLehrer(String fach)
    {
        ...
        {
            // SQL-Statement
            rs = stmt.executeQuery("SELECT "+ Konfig.KURS_LEHRER +" FROM "
                    + Konfig.KURS_TABELLE + " WHERE " +
                Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() + "' AND " +
                Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%' "
                + "AND " + Konfig.KURS_FACHBEZEICHNUNG + " LIKE '" + fach + "'");
            rs.next();
            // Lehrer Kürzel:
    ...
    public String gibLeistungskurs()
    {
        try
        {
            // SQL-Statement, um die Anzahl der Stunden für MatheLK herrauszufinden:
            // (Mathe wird es immer als Leistungskurs geben)
            rs = stmt.executeQuery("SELECT Kursstunden FROM " + Konfig.KURS_TABELLE +
                    " WHERE Kursart='L' AND Fachbezeichnung='Mathematik' GROUP BY"
                    + " Kursstunden");
            rs.next();
            int stunden = rs.getInt(1);
            // SQL-Statment, um durch die Stundenanzahl den LK herauszufinden:
            rs = stmt.executeQuery("SELECT Fachbezeichnung FROM Kurs WHERE Kursstunden = "
                + stunden + " AND " + Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() + "' AND " +
                Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%' " +  " GROUP BY Fachbezeichnung");
            rs.next();
            // Leistungskurs:
            return rs.getString(1);
        }
     ...
     * 
     * @param fach Fach, dessen Kursbezeichnung gesucht wird
     ...
     * 
     * @param fach Fach, dessen Kursstunden herrausgefunden werden sollen
    ...
    public int gibStunden(String fach)
    {
        ...
        {
            // SQL-Statement:
            rs = stmt.executeQuery("SELECT " + Konfig.KURS_KURSSTUNDEN + " FROM " +
                    Konfig.KURS_TABELLE + " WHERE " + Konfig.KURS_SCHULERNAMEN +
                    " LIKE '" + name.trim() + "' AND " + Konfig.KURS_KLASSENNAMEN +
                    " LIKE '%" + kurs + "%' AND " + Konfig.KURS_FACHBEZEICHNUNG +
                    "='" + fach + "'");
            rs.next();
            // Kursstunden:
            return rs.getInt(1);
     ...
     * übereinstimmt.
     * 
     * @param gebDatum Geburtsdatum, des überprüft werden soll.
    ...
    public boolean istGeburtsdatum(String gebDatum)
    {
        ...
        {
            // SQL-Statement:
            rs = stmt.executeQuery("SELECT Count(" + Konfig.SCHUELER_GEBDATUM +
                    ") FROM " + Konfig.SCHUELER_TABELLE + " WHERE " +
                    Konfig.SCHUELER_NAME + " LIKE '" + name + "' AND " +
                    Konfig.SCHUELER_KLASSE + " LIKE '%" + kurs + "%' AND " +
                    Konfig.SCHUELER_GEBDATUM + " ='" + gebDatum + "'");
            rs.next();
     ...
     * 
     * @param gebOrt Geburtsort, des überprüft werden soll.
    ...
    public boolean istGeburtsort(String gebOrt)
    {
        try
        {
            ...
            {
                // SQL-Statement
                rs = stmt.executeQuery("SELECT Count(" + Konfig.SCHUELER_GEBORT +
                        ") FROM " + Konfig.SCHUELER_TABELLE + " WHERE " +
                        Konfig.SCHUELER_NAME + " LIKE '" + name + "' AND " +
                        Konfig.SCHUELER_KLASSE + " LIKE '%" + kurs + "%' AND (" +
                        Konfig.SCHUELER_GEBORT + " LIKE '" + gebOrt + "%' OR " +
                        Konfig.SCHUELER_GEBORT +" = '" + gebOrt + "' )");
                rs.next();
    ...
    public void setzeFachrichtung()
    {
        try
        {
            // SQL-Statement, um zu überprüfen, ob der Schüler in einer Klasse mit
            // zwei Fachrichtungen ist:
            rs = stmt.executeQuery("SELECT Fachrichtung FROM Fachrichtung, Kurs WHERE " +
                    "Fachrichtung.Kurs = Kurs.Kursbezeichnung AND " +
                    Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() + "' AND " +
                    Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%' GROUP BY Fachrichtung");
            rs.next();
            // Fachrichtung:
        ...
        catch (SQLException ex)
        {
            try
            {
                ...
                rs = stmt.executeQuery("SELECT Fachrichtung FROM Fachrichtung," +
                    " Kurs WHERE Fachrichtung.Schulform = Kurs.USFBKSchueler AND " +
                    Konfig.KURS_SCHULERNAMEN + " LIKE '" + name.trim() + "' AND " +
                    Konfig.KURS_KLASSENNAMEN + " LIKE '%" + kurs + "%' GROUP BY Fachrichtung");
                rs.next();
                // Fachrichtung:
     ...
     *
     * @param kurs Kurs, in dem sich der Schüler befindet
     */
     ...
     *
     * @param updates String-Array mit {[Spaltenname], [Inhalt]}
    ...
    private boolean update(String[][] updates)
    {
        try
        {
            try
            {
                ...
                stmt.executeUpdate("INSERT INTO " + Konfig.ERGENIS_TABELLE +
                        " (Name, Kurs) VALUES ('" + name.replace("_","’") + "','" + kurs + "')");
            }
            ...
            stmt.executeUpdate("UPDATE " + Konfig.ERGENIS_TABELLE + " SET " +
                    update + " WHERE Name='" + name.replace("_","’") + "' AND Kurs='" + kurs + "'");
     ...
     * vermieden, dass Daten nur teilweise ankommen und somit nicht erkannt wird,
     * ob das Update geglückt ist.
     */
    public boolean update(String lk, String zahl, String nat, String fremd, boolean powi_bili,
                            boolean gesch, boolean powi, String sport, String reli, String wahlpflicht,
                            boolean hatLKNW, boolean gesch_bili)
    {
        ...
        if(powi)            intPowi = "-1";
        if(gesch)           intGesch = "-1";
        if(powi_bili)       intPowi_bili = "-1";
        if(gesch_bili)       intGesch_bili = "-1";
        if(hatLKNW)         intHatLKNW = "-1";        
        ...
        if(update(new String[][]
        {
            ...
            {"Physik", nat.equals("Physik")?"-1":"0"},
             ...
             {"Hat_Religion", reli.equals("Nicht teilnehmen")?"0":"-1"},
             {"reli_k", reli_k},
             {"reli_l", reli_l},
             
             {"Sportkurs", sport},
             {"Wahlpflichtkurs", wahlpflicht},
             {"klasseWiederholen", "0"},
             {"schuleVerlassen", "0"}
        }))
        {
